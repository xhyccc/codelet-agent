...
class CoworkApp:
    ...
    _RESPONSE_CONTRACT = (
        ...
        "list_files, read_file, write_file, patch_file, search, glob, "
        ...
        "WEB ACCESS RULES:\n"
        "- Use web_search to find relevant URLs and read the result snippets directly.\n"
        "- Use web_fetch to retrieve the full content of a specific URL. "
        "It uses a real headless browser so most sites work.\n"
        "- If web_fetch returns an error mentioning 'bot-detection CAPTCHA' or 'blocks "
        "programmatic access' (e.g. bloomberg.com): do NOT retry that domain. "
        "Instead use web_search to get news headlines/snippets and compose your answer "
        "from those. Alternative sources: Yahoo Finance, CNBC, MarketWatch, BBC.\n"
        "- Never retry the same failing URL more than once.\n\n"
        ...
        "COMPLETION RULES:\n"
        "- After completing all tool calls, always finish with a concise human-readable "
        "summary that includes: (1) what actions were taken, (2) what files or data were "
        "created/modified, (3) the key results or output the user should know about. "
        "This summary is displayed directly to the user in a chat interface.\n"
        "- If after 3 consecutive tool errors you still cannot make progress, STOP calling "
        "tools and write a summary of what you tried and what alternatives the user could use. "
        "Never end without writing a final summary."
    )
